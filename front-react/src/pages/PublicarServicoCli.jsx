...
const PublicarServico = () => {
  ...
  const categorias = ['Elétrica', 'Hidráulica', 'Pintura', 'Limpeza', 'Construção', 'Outros'];
  ...
  const validarFormulario = () => {
    if (!formData.titulo.trim()) { mostrarToast('Informe o título', true); return false; }
    if (!formData.descricao.trim()) { mostrarToast('Informe a descrição', true); return false; }
    if (!formData.categoria) { mostrarToast('Selecione uma categoria', true); return false; }
    if (!formData.preco || isNaN(formData.preco)) { mostrarToast('Informe um valor válido', true); return false; }
    if (!formData.contato.trim()) { mostrarToast('Informe um contato', true); return false; }
    if (!formData.localizacao.trim()) { mostrarToast('Informe sua localização', true); return false; }
    return true;
  };
    ...
    <>
      ...
      <div className="publish-container">
        <div className="form-header">
          <h1><i className="fas fa-plus-circle"></i> Publicar novo serviço</h1>
          <div className="subhead">Preencha os dados abaixo para divulgar seu serviço para milhares de clientes</div>
        </div>
        <form onSubmit={handleSubmit} className="form-content">
          <div className="field-group">
            <div className="field-label"><i className="fas fa-heading"></i> Título do serviço <span className="required-star">*</span></div>
            <input type="text" name="titulo" placeholder="Ex: Instalação elétrica residencial" value={formData.titulo} onChange={handleChange} />
          </div>
          <div className="field-group">
            <div className="field-label"><i className="fas fa-align-left"></i> Descrição detalhada <span className="required-star">*</span></div>
            <textarea name="descricao" placeholder="Descreva o serviço, materiais inclusos, garantia, etc." value={formData.descricao} onChange={handleChange} />
            <div className="hint-text">Seja o mais claro possível para atrair clientes qualificados.</div>
          </div>
          <div className="field-group">
            <div className="field-label"><i className="fas fa-camera"></i> Fotos do serviço</div>
              ...
              <div className="file-list">
                {fotos.map((foto, idx) => (
                  <div key={idx} className="file-tag">
                    <img src={foto.base64} alt={foto.name} style={{ width: '30px', height: '30px', objectFit: 'cover', borderRadius: '6px' }} />
                    {foto.name.length > 20 ? foto.name.substring(0, 17) + '...' : foto.name}
                    <i className="fas fa-times-circle" style={{ cursor: 'pointer', color: '#dc2626' }} onClick={(e) => { e.stopPropagation(); removerFoto(idx); }}></i>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="field-group">
            <div className="field-label"><i className="fas fa-tags"></i> Categoria <span className="required-star">*</span></div>
            <div className="radio-group">
              {categorias.map(cat => (
                <label key={cat} className={`radio-item ${formData.categoria === cat ? 'selected' : ''}`}>
                  <input type="radio" name="categoria" value={cat} checked={formData.categoria === cat} onChange={handleChange} style={{ display: 'none' }} />
                  {cat}
                </label>
              ))}
            </div>
          ...
          <div className="double-row">
            <div className="field-group">
              <div className="field-label"><i className="fas fa-dollar-sign"></i> Preço sugerido (R$) <span className="required-star">*</span></div>
              <input type="number" name="preco" placeholder="Ex: 150" value={formData.preco} onChange={handleChange} step="0.01" />
            </div>
            <div className="field-group">
              <div className="field-label"><i className="fas fa-bolt"></i> Urgência</div>
          ...
          <div className="double-row">
            <div className="field-group">
              <div className="field-label"><i className="fas fa-phone-alt"></i> Contato <span className="required-star">*</span></div>
              <input type="text" name="contato" placeholder="Telefone ou e-mail" value={formData.contato} onChange={handleChange} />
            </div>
            <div className="field-group">
              <div className="field-label"><i className="fas fa-map-marker-alt"></i> Localização <span className="required-star">*</span></div>
              <input type="text" name="localizacao" placeholder="Cidade, bairro ou região" value={formData.localizacao} onChange={handleChange} />
              <div className="hint-text">O cliente verá essa informação</div>
            </div>
          </div>
          <button type="submit" className="publish-btn"><i className="fas fa-paper-plane"></i> Publicar serviço</button>
        </form>
      </div>

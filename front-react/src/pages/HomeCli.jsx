...
const HomeCli = () => {
  ...
  useEffect(() => {
    const storedPrestadores = JSON.parse(localStorage.getItem('prestadores_cadastrados') || '[]');
    setPrestadores(storedPrestadores);
    const userProfile = JSON.parse(localStorage.getItem('userProfile') || '{}');
    if (userProfile.nome) setClienteNome(userProfile.nome);
    if (userProfile.cidade && userProfile.estado) setClienteLocal(`${userProfile.cidade} - ${userProfile.estado}`);
    else if (userProfile.endereco) setClienteLocal(userProfile.endereco.split(',')[0]);
      ...
      <div className="home-container">
        <div className="user-header">
          <div className="user-info">
            ...
            <div className="user-location"><i className="fas fa-map-marker-alt"></i> {clienteLocal}</div>
          </div>
          <div className="user-actions">
            <a href="propostas-cli.html" className="icon-btn" title="Notificações"><i className="fas fa-bell"></i></a>
            <a href="publicarServico-cli.html" className="icon-btn" title="Publicar Serviço"><i className="fas fa-plus-circle"></i></a>
            <a href="servicos-cli.html" className="icon-btn" title="Serviços Recebidos"><i className="fas fa-briefcase"></i></a>
            <a href="confPerfil-cli.html" className="icon-btn" title="Perfil"><i className="fas fa-user"></i></a>
            <a href="home-cli.html" className="icon-btn" title="Home"><i className="fas fa-home"></i></a>
            <a href="index.html" className="icon-btn" title="Sair"><i className="fas fa-sign-out-alt"></i></a>
          </div>
        </div>
        <div className="main-grid">
          <div className="professionals-section">
            <h3><i className="fas fa-users"></i> Profissionais próximos a você</h3>
            <div className="cards-grid">
              {prestadores.map((prof) => (
                <div key={prof.id} className="professional-card" onClick={() => openModal(prof.id)}>
                  <div className="card-header"><span className="prof-name">{prof.nome}</span><div className="rating"><i className="fas fa-star"></i> {prof.avaliacao || 'Novo'}</div></div>
                  <div className="service-title">{prof.profissao}</div>
                  <div className="service-description">{prof.bio ? prof.bio.substring(0, 60) : 'Profissional disponível para serviços.'}</div>
                  <div className="service-meta"><span><i className="fas fa-map-marker-alt"></i> {prof.localizacao || 'Local não informado'}</span><span className="price">💰 Sob consulta</span></div>
                </div>
              ))}
              {prestadores.length === 0 && (<div className="professional-card" style={{ textAlign: 'center' }}><p>Nenhum profissional cadastrado ainda.</p><p style={{ fontSize: '0.8rem' }}>Peça para um prestador se cadastrar no perfil.</p></div>)}
            </div>
          </div>
          <div className="announcement-section">
            <h3><i className="fas fa-tag"></i> Destaques</h3>
            <div className="announcement-card"><h4><i className="fas fa-hard-hat"></i> Material de Construção</h4><p>Promoções em ferramentas</p><p>Aproveite as ofertas da semana!</p><button className="announcement-btn" onClick={handleAnnouncement}>Ver ofertas →</button></div>
            <div className="announcement-card" style={{ background: "linear-gradient(135deg,#3b82f6 0%,#2563eb 100%)" }}><h4><i className="fas fa-broom"></i> Limpeza e Conservação</h4><p>Desconto especial para novos clientes</p><p>Agende já sua diarista</p><button className="announcement-btn" onClick={handleCleaning}>Agendar →</button></div>
          </div>
        </div>
        <div className="footer">
          <div className="footer-content">
            <div className="footer-col"><h4>Servijá</h4><p>Conectando pessoas a prestadores de serviços de forma rápida, prática e segura.</p><div className="social-links"><a href="#"><i className="fab fa-facebook-f"></i></a><a href="#"><i className="fab fa-instagram"></i></a><a href="#"><i className="fab fa-whatsapp"></i></a><a href="#"><i className="fab fa-twitter"></i></a></div></div>
            <div className="footer-col"><h4>Para clientes</h4><a href="#">Como funciona</a><a href="#">Profissionais disponíveis</a><a href="#">Avaliações</a><a href="#">Dúvidas frequentes</a></div>
            <div className="footer-col"><h4>Para profissionais</h4><a href="#">Cadastre-se</a><a href="#">Planos e preços</a><a href="#">Central do profissional</a><a href="#">Depoimentos</a></div>
            <div className="footer-col"><h4>Contato</h4><p><i className="fas fa-phone"></i> (61) 99999-9999</p><p><i className="fas fa-envelope"></i> contato@servija.com.br</p><p><i className="fas fa-clock"></i> Seg-Sex: 8h às 18h</p></div>
          </div>
          <div className="footer-bottom"><p>&copy; 2024 Servijá - Todos os direitos reservados.</p></div>
        </div>
        <div className={`modal ${modalOpen ? 'active' : ''}`} onClick={handleOverlayClick}>
          <div className="modal-content">
            <div className="modal-header"><h3><i className="fas fa-user-circle"></i> Detalhes do Profissional</h3><button className="close-modal" onClick={closeModal}>&times;</button></div>
            <div className="modal-body">
              {selectedProfessional && (
                <>
                  <div className="modal-prof-name">{selectedProfessional.nome}</div>
                  <div className="modal-rating">⭐ {selectedProfessional.avaliacao || 'Novo'}</div>
                  <div className="modal-info">
                    <p><strong>Profissão:</strong> {selectedProfessional.profissao}</p>
                    <p><strong>Experiência:</strong> {selectedProfessional.experiencia || 'Não informada'}</p>
                    <p><strong>Habilidades:</strong> {selectedProfessional.habilidades?.join(', ') || 'Não informadas'}</p>
                    <p><strong>Localização:</strong> {selectedProfessional.localizacao || 'Não informada'}</p>
                  </div>
                  <button className="contact-btn" onClick={handleContact}>Entrar em contato</button>
                </>
              )}
            </div>
          </div>
        </div>
        {toast.visible && (<div className="success-toast" style={{ background: toast.isError ? '#dc2626' : '#f97316' }}>{toast.message}</div>)}
      </div>
